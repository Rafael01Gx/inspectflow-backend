...
public class GlobalExceptionHandler {
    ...
    @ExceptionHandler(UnauthorizedProfessionalException.class)
    public ResponseEntity<ErrorResponse> handleUnauthorizedProfessional(UnauthorizedProfessionalException ex) {
        ErrorResponse error = ErrorResponse.builder()
                .status(HttpStatus.FORBIDDEN.value())
                .message(ex.getMessage())
                .timestamp(LocalDateTime.now())
                .build();
        log.warn("UnauthorizedProfessionalException: {}", ex.getMessage());
        return ResponseEntity.status(HttpStatus.FORBIDDEN).body(error);
    }

    @ExceptionHandler(WorkerOrderNotFoundException.class)
    public ResponseEntity<ErrorResponse> handleWorkerOrderNotFound(WorkerOrderNotFoundException ex) {
        ErrorResponse error = ErrorResponse.builder()
                .status(HttpStatus.NOT_FOUND.value())
                .message(ex.getMessage())
                .timestamp(LocalDateTime.now())
                .build();
        log.warn("WorkerOrderNotFoundException: {}", ex.getMessage());
        return ResponseEntity.status(HttpStatus.NOT_FOUND).body(error);
